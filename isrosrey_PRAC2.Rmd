...
Se considera que todas las columnas pueden aportar información de interés, pero 
más adelante se descartará el campo "Cabin" porque tiene muchos elementos vacíos
que serían difíciles de adivinar para su imputación. 
Por otro lado, el campo "PassengerId" no será útil para determinar si un pasajero
sobrevive o muere, pero se usará para recomponer los bloques de entrenamiento y
prueba originales al finalizar la limpieza de los datos.
Se podría intentar crear algún campo adicional que sea de utilidad, por ejemplo,
un campo que agrupe los miembros de una misma familia, si un pasajero es adulto
o no, etc.
...
"Survived" que ya sabemos que los datos de test omitían este dato), y tenemos
elementos vacíos en las variables "Cabin" y "Embarked". Observando los datos
ordenados por el Cabin, haciendo uso del visor de RStudio, nos damos cuenta de
que los pasajeros con Cabin B19 al B26 han embarcado por la puerta de Southampton,
por tanto, suponemos que a los 2 valores vacíos que tenían la cabina B28 asiganda,
les correspondería entrar por la puerta S también.

```{r}
merged_data$Embarked[merged_data$Embarked == ""] <- "S"
```

Una vez hemos completado la variable "Embarked" basándonos en los valores del
Cabin, eliminamos la columna "Cabin" y asignamos la media de edad de las mujeres
y la de los hombres:
...
# por ejemplo)
boxplot(merged_data$Age, names="Edad", main="Edades")
...
g3 <- ggplot(merged_data, aes(y=Parch)) + geom_boxplot()
g4 <- ggplot(merged_data, aes(y=Fare)) + geom_boxplot()
...
```{r}
# Comprobamos la normalidad con 2 test, el de Kolmogorov-Smirnov y de Shapiro-Wilk.
ks.test(merged_data$Age, pnorm, mean(merged_data$Age), sd(merged_data$Age))
shapiro.test(merged_data$Age)
# No sigue distribución normal por tener p-value inferior a 0.05
```
```{r}
# Comprobamos la homocedasticidad
# Test de Levene para casos que cumplen la normalidad
leveneTest(Age ~ Survived, data = merged_data)
# Test de Fligner-Killeen para los que no cumplen la condición de normalidad
fligner.test(Age ~ Survived, data = merged_data)
# Tiene valor inferior a 0.05 por lo que indica heterocedasticidad
```
